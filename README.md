...
| `checkout`        | _`boolean \| Partial<CheckoutActionSettings>`_  | `true`  | If _truthy_, `@actions/checkout` is triggered. If `checkout` is a [`CheckoutActionSettings`-like object][43], it is passed to `@actions/checkout` as configuration. Note that [`checkout.persistCredentials`][43] is set to `false` by default. Note also the additional [`checkout.bareWorkingTree`][43] setting, which is false by default for this component action. |
| `setupNode`       | _`boolean \| Partial<SetupNodeActionSettings>`_ | `true`  | If _truthy_, `@actions/setup-node` is triggered. If `setupNode` is a [`SetupNodeActionSettings`-like object][43], it is passed to `@actions/setup-node` as configuration. See also: [configuring the pipeline][23].                                                                                                                                                     |
